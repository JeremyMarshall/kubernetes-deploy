...
module KubernetesDeploy
  class RunnerTask
    ...
    def run!(task_template:, entrypoint:, args:, env_vars: [], verify_result: true)
      ...
      StatsD.distribution('task_runner.duration', StatsD.duration(start),
        tags: statsd_tags('success'), prefix: "KubernetesDeploy")
      @logger.print_summary(:success)
    rescue DeploymentTimeoutError
      StatsD.distribution('task_runner.duration', StatsD.duration(start),
        tags: statsd_tags('timeout'), prefix: "KubernetesDeploy")
      @logger.print_summary(:timed_out)
      raise
    rescue FatalDeploymentError
      StatsD.distribution('task_runner.duration', StatsD.duration(start),
        tags: statsd_tags('failure'), prefix: "KubernetesDeploy")
